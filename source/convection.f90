...
contains
    ...
    subroutine get_convection_tendencies(state, psa, se, qa, qsat, itop, dfse, dfqa)
        ...
        fm0 = p0*state%dhs(kx)/(grav*trcnv*3600.0)
        rdps = 2.0/(1.0 - psmin)
        ...
        sentr = 0.0
        do k = 2, nl1
            entr(k) = (max(0.0, state%fsg(k) - 0.5))**2.0
            sentr = sentr + entr(k)
        end do
        ...
        do i = 1, ix
            do j = 1, il
                ...
                do k = kx - 1, itop(i, j) + 1, -1
                    ...
                    ! Mass entrainment
                    enmass = entr(k)*psa(i, j)*state%cbmf(i, j)
                    fmass = fmass + enmass
                ...
                state%precnv(i, j) = max(fuq - fmass*qsatb, 0.0)

                ! Net flux of dry static energy and moisture
                dfse(i, j, k) = fus - fds + alhc*state%precnv(i, j)
                dfqa(i, j, k) = fuq - fdq - state%precnv(i, j)
            end do
        end do
    end

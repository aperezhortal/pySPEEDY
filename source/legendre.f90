...
contains
    ...
    function get_legendre_poly(j, epsi, repsi) result(poly)
        use geometry, only : sia_half, coa_half
        real(p), intent(in) :: epsi(mx + 1, nx + 1)   !! Epsilon function used for various spectral calculations
        real(p), intent(in) :: repsi(mx + 1, nx + 1)   !! 1/epsi

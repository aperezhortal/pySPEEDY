...
contains
    ...
    subroutine get_vertical_diffusion_tend(state, se, rh, qa, qsat, phi, icnv, utenvd, vtenvd, &
                                           ttenvd, qtenvd)
        ...
        do k = 1, nl1
            rsig(k) = 1.0/state%dhs(k)
            rsig1(k) = 1.0/(1.0 - sigh(k))
        end do
        rsig(kx) = 1.0/state%dhs(kx)
        ...
        drh0 = rhgrad*(state%fsg(kx) - state%fsg(nl1))
        ...
        do k = 3, kx - 2
            if (sigh(k) > 0.5) then
                drh0 = rhgrad*(state%fsg(k + 1) - state%fsg(k))
                fvdiq2 = fvdiq*sigh(k)

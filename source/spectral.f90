...
contains
    ...
    subroutine initialize_spectral(state)
        ...
        call initialize_legendre(state)
    ...
    function spec_to_grid(vorm, kcos, cosgr) result(vorg)
        ...
        vorg = fourier_inv(legendre_inv(vorm_r), kcos, cosgr)
    end function
    ...
    subroutine vdspec(ug, vg, kcos, cosgr, cosgr2, vorm, divm)
        real(p), intent(in) :: ug(ix, il), vg(ix, il)
        real(p), intent(in) :: cosgr(il), cosgr2(il)
        integer, intent(in) :: kcos

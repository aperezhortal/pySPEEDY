...
contains
    ...
    subroutine initialize_implicit(state, dt)
        ...
        do k = 1, kx
            tref(k) = 288.*max(0.2, state%fsg(k))**rgam
            tref1(k) = rgas*tref(k)
            tref2(k) = akap*tref(k)
            tref3(k) = state%fsgr(k)*tref(k)
        end do
        ...
        do k = 1, kx
            do k1 = 1, kx
                ya(k, k1) = -akap*tref(k)*state%dhs(k1)
            end do
        end do

        do k = 2, kx
            xa(k, k - 1) = 0.5*(akap*tref(k)/state%fsg(k) - (tref(k) - tref(k - 1))/state%dhs(k))
        end do

        do k = 1, kx - 1
            xa(k, k) = 0.5*(akap*tref(k)/state%fsg(k) - (tref(k + 1) - tref(k))/state%dhs(k))
        end do

        !sig(k)=xb(k,k')*d(k')
        dsum(1) = state%dhs(1)
        do k = 2, kx
            dsum(k) = dsum(k - 1) + state%dhs(k)
        end do

        do k = 1, kx - 1
            do k1 = 1, kx
                xb(k, k1) = state%dhs(k1)*dsum(k)
                if (k1 .le. k) xb(k, k1) = xb(k, k1) - state%dhs(k1)
            end do
        end do
        ...
        do k = 1, kx
            do k1 = k + 1, kx
                xd(k, k1) = rgas*log(state%hsg(k1 + 1)/state%hsg(k1))
            end do
        end do
        do k = 1, kx
            xd(k, k) = rgas*log(state%hsg(k + 1)/state%fsg(k))
        end do
        ...
        do l = 1, mx + nx + 1
            ...
            do k = 1, kx
                do k1 = 1, kx
                    xf(k, k1, l) = xi*xi*xxx*(rgas*tref(k)*state%dhs(k1) - xe(k, k1))
                end do
            end do
            do k = 1, kx
                xf(k, k, l) = xf(k, k, l) + 1.
